...
On RTX 4060, the matched IN718 10 µm, 73,568-cell, 934-step case passed CPU/GPU parity with both source paths: field-rise L2/max relative differences were `2.60e-9` / `7.28e-9`, and energy/other parity targets passed. Three alternating direct solver runs gave medians of 11.4155 s for the CPU reference, 9.4337 s for CUDA evolution + CPU source integration, and 9.4236 s for CUDA evolution + CUDA source integration. This is about 17.4% shorter GPU-pilot runtime than the CPU reference on this workload; moving source integration to CUDA adds no meaningful improvement over CPU source integration. The earlier 17.652/19.630 s `compare_with_cpu` measurements included both GPU and CPU oracle work, so they are not standalone path timings. A 5 µm full run did not start because it exceeds the pilot's current `MAX_CELLS=100000` guard; earlier 5 µm component timing is not a full-solver result. Keep the CPU source path as default and CUDA source integration opt-in. These are numerical/software checks, not experimental validation.

At 20 µm and 9,196 cells, three direct measurements gave 6.8944 s for CUDA evolution + CPU source and 3.3405 s for the CPU reference (GPU about 2.06x slower). The prior full CPU/GPU field comparison at that mesh passed. The measured crossover is bounded only between 9,196 and 73,568 cells; no intermediate threshold has been demonstrated.
