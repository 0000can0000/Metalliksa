...
On RTX 4060, the matched IN718 10 µm, 73,568-cell, 934-step full transient passed CPU/GPU parity with both source paths: CPU source + CUDA evolution took 17.652 s; CUDA source + CUDA evolution took 19.630 s. Field-rise L2/max relative differences were `2.60e-9` / `7.28e-9`; energy closure and the remaining parity targets passed. This single pair shows no end-to-end CUDA-source speedup. A 5 µm full run did not start because it exceeds the pilot's current `MAX_CELLS=100000` guard; earlier 5 µm component timing is not a full-solver result. Keep the CPU source path as default and CUDA source integration opt-in. These are numerical/software checks, not experimental validation.
