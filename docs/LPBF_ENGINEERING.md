...
Field-series frames optionally contain backend-derived projected molten-cell bounds and scan angle. They support current-snapshot L/W/D guides; the header still reports the largest sampled molten volume. The viewer reconstructs a liquidus section by linear interpolation on triangles joining the nearest section's cell centers. This is a temperature isotherm, not a metal/gas surface. Rendering uses exact-size cell cubes, a translucent mushy zone and the same fixed color mapping as the legend. Large fields retain explicit display sampling; the section contour uses all section cells. No sub-cell geometric accuracy is claimed by the smoother contour. Peak sample and all-step peak temperature can differ and are labelled separately.
