...
def run_inverse_alloy_optimizer(target_yield_mpa=1100.0, max_density_g_cm3=8.2, max_cost_usd_kg=40.0,
    ...
    """
    Genetic Algorithm optimizing alloy composition for multi-objective targets.
    """
    start_time = time.perf_counter()
    
    if not allowed_elements or len(allowed_elements) < 3:
        allowed_elements = ["Ni", "Cr", "Co", "Mo", "W", "Al", "Ti", "Nb", "Fe"]
    ...
    for _ in range(population_size):
        ...
        for el in allowed_elements:
            if el == "Ni":
                cand[el] = random.uniform(40.0, 75.0)
            elif el in ["Cr", "Co", "Fe"]:
                cand[el] = random.uniform(5.0, 25.0)
            elif el in ["Mo", "W", "Al", "Ti", "Nb", "Ta"]:
                cand[el] = random.uniform(0.5, 6.0)
            else:
                cand[el] = random.uniform(0.0, 2.0)
        population.append(cand)

    def compute_fitness(cand_comp):
        eval_res = evaluate_alloy_candidate(cand_comp)
        if not eval_res:
            return -1e6, None
        
        # Penalties for violating user constraints
        score = 100.0
        
        # Yield Strength objective
        ys = eval_res["yieldStrength_MPa"]
        if ys >= target_yield_mpa:
            score += min(50.0, (ys - target_yield_mpa) * 0.1)
        else:
            score -= (target_yield_mpa - ys) * 0.5
            
        # Density objective
        dens = eval_res["density_g_cm3"]
        if dens <= max_density_g_cm3:
            score += (max_density_g_cm3 - dens) * 15.0
        else:
            score -= (dens - max_density_g_cm3) * 60.0
            
        # Cost objective
        cost = eval_res["cost_USD_kg"]
        if cost <= max_cost_usd_kg:
            score += (max_cost_usd_kg - cost) * 2.0
        else:
            score -= (cost - max_cost_usd_kg) * 5.0
            
        # PHACOMP Nv constraint
        nv = eval_res["phacomp_Nv"]
        if nv <= max_phacomp_nv:
            score += 20.0
        else:
            score -= (nv - max_phacomp_nv) * 250.0
            
        # PREN objective
        pren_val = eval_res["pren"]
        if pren_val >= min_pren:
            score += 15.0
        else:
            score -= (min_pren - pren_val) * 2.0
    ...
    for gen in range(generations):
        ...
        for cand in population:
            score, res = compute_fitness(cand)
            scored_pop.append((score, cand, res))
        ...
        convergence_history.append({
            "generation": gen + 1,
            "bestFitness": round(best_score, 2),
            "bestYield_MPa": scored_pop[0][2]["yieldStrength_MPa"] if scored_pop[0][2] else 0.0,
            "bestDensity": scored_pop[0][2]["density_g_cm3"] if scored_pop[0][2] else 0.0,
            "bestCost": scored_pop[0][2]["cost_USD_kg"] if scored_pop[0][2] else 0.0
        })
        
        # Elitism: retain top 25%
        survivors = [x[1] for x in scored_pop[:population_size // 4]]
        ...
        while len(new_pop) < population_size:
            ...
            for el in allowed_elements:
                ...
                w2 = p2.get(el, 0.0)
                alpha = random.random()
                child_val = alpha * w1 + (1.0 - alpha) * w2
                
                # Mutation (15% probability)
                if random.random() < 0.15:
                    child_val += random.gauss(0.0, 1.5)
                child[el] = max(0.0, child_val)
            new_pop.append(child)
            
        population = new_pop

    # Final Evaluation & Extract Top Pareto Solutions
    final_scored = []
    for cand in population:
        score, res = compute_fitness(cand)
        if res:
            final_scored.append((score, res))
            
    final_scored.sort(key=lambda x: x[0], reverse=True)
    
    # De-duplicate top candidates
    unique_candidates = []
    seen_compositions = set()
    
    for score, res in final_scored:
        comp_key = "-".join(f"{k}:{v}" for k, v in sorted(res["composition"].items()))
    ...
    return {
        "success": True,
        "engine": "MetalliX-Python-HPC-InverseAlloy-v3.10",
        "computeTimeMs": compute_time_ms,
        "targetConstraints": {
            "targetYield_MPa": target_yield_mpa,
            "maxDensity_g_cm3": max_density_g_cm3,
            "maxCost_USD_kg": max_cost_usd_kg,
            "maxPHACOMP_Nv": max_phacomp_nv,
            "minPREN": min_pren
        },
        "topCandidate": unique_candidates[0] if unique_candidates else None,
        "paretoCandidates": unique_candidates,
