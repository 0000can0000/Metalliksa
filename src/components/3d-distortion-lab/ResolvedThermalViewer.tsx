...
export function ResolvedThermalViewer({jobId}:{jobId:string}) {
  ...
  useEffect(()=>{
    ...
    (async()=>{const s=parseFieldSeries(await (await get("field-series.json")).json());const c=decodeField(await (await get(s.coordinates)).arrayBuffer(),s.cells*3);if(!abort.signal.aborted){setSeries(s);setCoordinates(c);}})().catch(e=>{if(!abort.signal.aborted)setError(e.message);});
    return()=>abort.abort();
  },[base]);
  useEffect(()=>{
  ...
  useEffect(()=>{
    ...
    renderer.domElement.setAttribute("aria-label","Resolved 3D thermal cell field");
    ...
    for(let i=0;i<series.cells;i+=stride){
      ...
      colour.setHSL(.64*(1-f),.85,.3+.3*f);positions.push(x,y,z);colors.push(colour.r,colour.g,colour.b);
    }
    ...
    const material=new THREE.MeshBasicMaterial();const mesh=new THREE.InstancedMesh(geometry,material,positions.length/3);const matrix=new THREE.Matrix4();
    ...
    const observer=new ResizeObserver(resize);observer.observe(element);resize();let raf=0;
    const render=()=>{controls.update();renderer.render(scene,camera);raf=requestAnimationFrame(render);};render();
    return()=>{cameraState.current={position:camera.position.clone(),target:controls.target.clone()};cancelAnimationFrame(raf);observer.disconnect();controls.dispose();geometry.dispose();material.dispose();axes.geometry.dispose();(axes.material as THREE.Material).dispose();box.geometry.dispose();(box.material as THREE.Material).dispose();renderer.renderLists.dispose();};
  },[series,coordinates,values,loadedIndex,quantity,section,hotOnly,reset]);
  ...
  return <section aria-label="Resolved thermal field explorer" className="overflow-hidden rounded-xl bg-[#080f1c] ring-1 ring-slate-700/70">
    ...
    <div className="flex flex-wrap items-center gap-4 text-sm"><label className="flex items-center gap-2">Y section<input aria-label="Y section" type="range" min="0" max="100" value={section} onChange={e=>setSection(Number(e.target.value))}/>{section}%</label><label><input type="checkbox" checked={hotOnly} onChange={e=>setHotOnly(e.target.checked)}/> Mushy / liquid cells only</label><button className="underline text-slate-300" onClick={()=>{cameraState.current=undefined;setReset(v=>v+1);}}>Reset camera</button></div>
    <div className="h-2 rounded bg-gradient-to-r from-blue-600 via-emerald-400 to-red-400"/><p className="text-xs text-slate-400">{quantity==="phase"?"0 solid → 1 liquid; enthalpy fraction, not a gas interface":series?`${Math.min(...series.frames.map(f=>f.minimum_K)).toFixed(0)} → ${Math.max(...series.frames.map(f=>f.maximum_K)).toFixed(0)} K · fixed scale across all frames`:""}. X red · Y green · Z blue. Drag to orbit; scroll to zoom.</p>
