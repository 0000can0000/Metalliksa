...
export function StochasticUQMMPDSStudio() {
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="px-2 py-0.5 rounded-full text-[10px] font-mono bg-emerald-500/20 text-emerald-300 border border-emerald-500/30">
                N={uqResult ? uqResult.sampleSizeN.toLocaleString() : mcSamples.toLocaleString()} simulated draws
              </span>
            </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="font-bold text-slate-200 text-xs">
                  {uqResult.samplingMetadata.samplingMethod === "sobol_qmc"
                    ? "Sobol Sequence Sampling Diagnostics"
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 pt-2.5 text-[11px]">
              ...
              <div className="p-2 rounded-lg bg-slate-900/60 border border-slate-800">
                ...
                <div className="font-bold text-sky-300 mt-0.5">
                  ...
                  <span className="text-[9px] text-slate-400 font-normal ml-1">(first {uqResult.samplingMetadata.discrepancySampleSize ?? 150} points)</span>
                </div>
              </div>
              <div className="p-2 rounded-lg bg-slate-900/60 border border-slate-800">
              ...
              <div className="p-2 rounded-lg bg-slate-900/60 border border-slate-800">
                ...
                <div className="font-bold text-purple-300 mt-0.5">
                  ...
                  <span className="text-[9px] text-slate-400 font-normal ml-1">(solver)</span>
                </div>
              </div>
            </div>
          </div>
      ...
      {uqResult?.samplingMetadata?.diagnosticsLimitations && (
        <p className="text-xs text-slate-400">{uqResult.samplingMetadata.diagnosticsLimitations}</p>
      )}
                ...
                <div className="p-2 rounded-xl bg-slate-800/40 border border-slate-700/50">
                  ...
                  {activeStats.aBasisConfidenceInterval95 && (
                    <span className="text-[9px] text-sky-400/80 block mt-0.5">
                      Approx. iid-normal 95% CI: [{activeStats.aBasisConfidenceInterval95[0]}–{activeStats.aBasisConfidenceInterval95[1]}]
                    </span>
                  )}
                </div>
                <div className="p-2 rounded-xl bg-slate-800/40 border border-slate-700/50">
                  ...
                  {activeStats.bBasisConfidenceInterval95 && (
                    <span className="text-[9px] text-emerald-400/80 block mt-0.5">
                      Approx. iid-normal 95% CI: [{activeStats.bBasisConfidenceInterval95[0]}–{activeStats.bBasisConfidenceInterval95[1]}]
                    </span>
                  )}
                </div>
              </div>
            ...
            <div className="lg:col-span-4 p-5 rounded-2xl bg-slate-900/90 border border-slate-800 space-y-4">
              ...
              <div className="p-3 rounded-xl bg-slate-800/50 border border-slate-700/60 font-mono text-xs space-y-1">
                <div className="flex justify-between text-slate-300">
                  <span>Central 95% of simulated draws:</span>
        ...
        <div className="p-5 rounded-2xl bg-slate-900/90 border border-amber-500/30 space-y-5">
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">
                Estimates first-order (S_i) and total-order (S_Ti) effects on simulated yield-strength scatter. Raw finite-sample estimates are not normalized and may fall outside [0, 1].
              </p>
            </div>
            <div className="flex items-center gap-2">
          ...
          <div className="h-72 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={uqResult.sobolSensitivityAnalysis} layout="vertical" margin={{ left: 20, right: 30, top: 10, bottom: 10 }}>
                ...
                <Tooltip
                  contentStyle={{ backgroundColor: "#0f172a", borderColor: "#334155", borderRadius: "12px", fontSize: "11px" }}
                  formatter={(val: number) => [`${val}%`, "100 × first-order estimate"]}
                />
            ...
            <table className="w-full text-xs font-mono text-left">
              <thead className="text-[10px] text-slate-400 uppercase bg-slate-900/80 border-b border-slate-800">
                <tr>
                  ...
                  <th className="py-2.5 px-3 text-purple-300">Index Difference (S_Ti - Sᵢ)</th>
                  <th className="py-2.5 px-3 text-right">100 × Sᵢ (%)</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-800">
                {uqResult.sobolSensitivityAnalysis.map((s, idx) => (
                  <tr key={idx} className="hover:bg-slate-800/30">
                    ...
                    <td className="py-2 px-3 text-amber-300 font-bold">{s.sobolFirstOrderIndex?.toFixed(3) ?? "N/A"}</td>
                    <td className="py-2 px-3 text-sky-300 font-bold">{s.sobolTotalOrderIndex?.toFixed(3) ?? "N/A"}</td>
                    <td className="py-2 px-3 text-purple-300 font-semibold">
                      {s.interactionIndex?.toFixed(3) ?? "N/A"}
                    </td>
                    <td className="py-2 px-3 text-right font-bold text-slate-200">{s.varianceContributionPct == null ? "N/A" : `${s.varianceContributionPct.toFixed(1)}%`}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="p-4 rounded-xl bg-slate-800/60 border border-slate-700/60 space-y-2">
            ...
            <p className="text-xs text-slate-300 leading-relaxed font-mono">
              {uqResult.sensitivityMetadata?.limitations ?? "Sensitivity estimates describe the assumed model inputs, not measured material variability."} Use sensitivity estimates to plan controlled experiments; qualification requires representative traceable test populations.
            </p>
          </div>
        </div>
      )}
      ...
      {activeTab === "mmpds-table" && uqResult && (
        <div className="p-5 rounded-2xl bg-slate-900/90 border border-emerald-500/30 space-y-4">
            ...
            <table className="w-full text-xs font-mono text-left">
              <thead className="text-[10px] text-slate-400 uppercase bg-slate-800/60 border-b border-slate-700">
                <tr>
                  ...
                  <th className="py-2.5 px-3 text-sky-400">A-Style Approx. CI</th>
                  <th className="py-2.5 px-3 text-emerald-300 font-bold">B-Basis (T₉₀)</th>
                  <th className="py-2.5 px-3 text-emerald-400">B-Style Approx. CI</th>
          ...
          <div className="p-3 rounded-xl bg-slate-800/40 border border-slate-700/50 text-[11px] font-mono text-slate-400 flex items-center justify-between flex-wrap gap-2">
            <span>
              {uqResult.stochasticProperties.yieldStrength_Rp02.allowableUncertaintyMethod}
            </span>
            <span className="text-slate-300">
              Approximate normal tolerance factors: k_A = {uqResult.stochasticProperties.yieldStrength_Rp02.mmpds_kA}, k_B = {uqResult.stochasticProperties.yieldStrength_Rp02.mmpds_kB}
            </span>
          </div>
        </div>
      )}
