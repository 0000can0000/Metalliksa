...
export function composeIndustrialVerdict(result: PythonLPBFResult): IndustrialVerdict {
  ...
  const widthOverHatch = W / Math.max(1, h);
  const depthOverLayer = D / Math.max(1, t);
  ...
  } else if (lofWarn || keyholeHigh || recoaterHigh || distortionHigh) {
    verdict = "risky";
  }
